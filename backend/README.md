...
> Part of the [Campus Event App](../README.md) monorepo — see the [root README](../README.md) for the project overview, and [`../frontend`](../frontend/README.md) for the Flutter client.
...
│   │   ├── index.dart               # GET /events              (feed / filter / search / paginate; ?upcoming=true)
│   │   ├── featured.dart            # GET /events/featured     (soonest N upcoming)
│   │   ├── tags.dart                # GET /events/tags         (distinct tag list)
│   │   ├── cover-image.dart         # POST /events/cover-image (Cloudinary upload → URL)
│   │   ├── created.dart             # GET /events/created      (caller's own events)
│   │   ├── registered.dart          # GET /events/registered   (?filter=upcoming|past, default upcoming)
│   │   ├── next-registered.dart     # GET /events/next-registered
│   │   ├── pending.dart             # GET /events/pending      (faculty review queue)
│   │   ├── rejected.dart            # GET /events/rejected     (faculty reopen queue)
│   │   └── [eventId]/
│   │       ├── index.dart           # GET /events/{eventId} (incl. is_registered) · PATCH · DELETE (soft)
│   │       ├── register/
│   │       │   └── index.dart       # POST /events/{eventId}/register · DELETE (cancel)
│   │       └── status/
│   │           └── index.dart       # PATCH /events/{eventId}/status (approve|reject|reopen)
│   └── users/
│       ├── _middleware.dart         # token verify → user lookup → deactivation → role
│       ├── index.dart               # GET /users?search=&role= (faculty/super_admin)
│       ├── me/
...
│   │   └── event_exception.dart     # EventException
...
│   ├── services/
│   │   ├── auth_service.dart
│   │   ├── cloudinary_service.dart          # server-side signed uploads
│   │   ├── event_service.dart               # feed/search/featured + in-memory snapshot cache
│   │   ├── event_moderation_service.dart    # pending/rejected queues + status changes
│   │   ├── firebase_auth_service.dart
│   │   ├── firebase_event_service.dart
│   │   ├── firebase_id_token_verifier.dart  # local RS256 ID-token verification
│   │   ├── firestore_client.dart            # shared token-caching Firestore client
│   │   ├── hardened_http_client.dart        # fresh-connection retry client
│   │   ├── registration_service.dart        # sole writer of the registrations collection
